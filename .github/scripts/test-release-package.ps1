...
$fixture = @'
<div align="center">
# ![GMTools](https://readme-typing-svg.demolab.com/?lines=GMTools)
...
Preserve `/gm <name>`, `<details>`, and [ordinary links](https://example.com/a_(b)).
</details>
```html
...
foreach ($want in @('# GMTools', '### Feature', 'Keep the summary.', '[Download](https://example.com/releases)',
    '`/gm <name>`', '`<details>`', '[ordinary links](https://example.com/a_(b))', '<details>Preserve this code.</details>')) {
...
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('gmtools-release-test-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($name in @('DEV_TRACKING.md', 'DISCORD_POST.md', 'Screenshot.png', 'local.lua.bak', 'local.spans.json',
        '.env', 'tools/private.lua', 'tests/capture.txt', 'data/local.db', 'data/local.db-wal')) {
        $dest = Join-Path $staged $name
        [IO.Directory]::CreateDirectory((Split-Path $dest -Parent)) | Out-Null
    ...
    foreach ($required in @('gmtools.lua', 'README.md', 'LICENSE')) {
